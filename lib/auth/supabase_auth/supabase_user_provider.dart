...
class CivjSupabaseUser extends BaseAuthUser {
  CivjSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> civjSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = CivjSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
